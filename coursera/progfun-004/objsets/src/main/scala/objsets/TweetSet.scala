...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val accSet = right.filterAcc(p, left.filterAcc(p, acc))
    if ( p(elem) ) accSet.incl(elem)
    else accSet
  }
  ...
  def compare_(t1: Tweet, t2: Tweet) = {
  ...
  def mostRetweeted: Tweet = {
    if (left.isEmpty && right.isEmpty) {
      ...
      compare_(elem, right.mostRetweeted)
    } else if (!left.isEmpty && right.isEmpty) {
      compare_(elem, left.mostRetweeted)
    } else {
      compare_(compare_(elem, left.mostRetweeted), right.mostRetweeted)
    }
  }
