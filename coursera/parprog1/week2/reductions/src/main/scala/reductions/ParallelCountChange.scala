...
object ParallelCountChange {
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int = {
    if (threshold(money, coins)) countChange(money, coins)
      ...
      case _ if (money < 0) => 0
      case x :: xs =>
        val (hasX, hasNoX) = parallel(parCountChange(money-x, coins, threshold), parCountChange(money, xs, threshold))
        hasX + hasNoX
      case _ if (money == 0) => 1
      case _ => 0
    }
  ...
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold = {
    (money, coins) => coins.length * money <= startingMoney * allCoins.length / 2
  }
}
