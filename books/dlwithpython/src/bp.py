...
class Node(object):
    ...
    def append_upstream_connecion(self, conn):
...
class Connection(object):
    ...
    def update_weight(self, rate):
        self.calc_gradient()
        self.weight += rate * self.get_gradient()
...
class Network(object):
    ...
    def train(self, dataset, labels, rate, iteration):
        for i in range(iteration):
            for j in range(len(dataset)):
                sample = dataset[j]
                label = labels[j]
                self.train_one_sample(sample, label, rate)
